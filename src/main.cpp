...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        for (const auto& instrument : strat.GetInstruments())
        {
            poco_information_f1(logger, "Loading existing orders for %s", instrument);
            m_orderManager->LoadOpenOrders(UTILS::CurrencyPair(instrument));
        }
